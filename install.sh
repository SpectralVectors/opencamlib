...
install_ci_dependencies() {
    if [ "${determined_os}" = "windows" ]; then
        ...
        temp_dir="/tmp"
        #cp "/opt/homebrew/Cellar/libomp/22.1.0" "${temp_dir}/libomp.tar.gz"
        mkdir -p "${temp_dir}/libomp/libomp/fixed" || true
        ...
        # mv  "/opt/homebrew/Cellar/libomp/22.1.0/lib/libomp.dylib" '/tmp/libomp/libomp/fixed/lib/libomp.dylib' # '/tmp/libomp/libomp/fixed'
        ls /opt/homebrew/Cellar/libomp/22.1.0
        cp '/opt/homebrew/Cellar/libomp/22.1.0/libomp.dylib'  '/tmp/libomp/libomp/fixed/lib'
